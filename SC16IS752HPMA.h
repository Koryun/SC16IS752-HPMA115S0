...
#ifndef _SC16IS752HPMA_H_
#define _SC16IS752HPMA_H_
...
// unsigned char SUCCESS[] = {0xA5, 0xA5};
...
// class SC16IS752HPMA : public Stream
class SC16IS752HPMA {
public:

  SC16IS752HPMA(uint8_t prtcl = SC16IS750_PROTOCOL_I2C,
  ...
  void initHPMA(uint8_t channel);
  void sendHPMACmd(uint8_t channel, uint8_t * cmdBuf, unsigned int cmdSize);
  int readHPMAResp(uint8_t channel, unsigned char * dataBuf, unsigned int dataBufSize, unsigned int cmdType);
  boolean readHPMAParticleMeasurement(uint8_t channel, unsigned int * pm2_5, unsigned int * pm10);
  void startHPMAParticleMeasurement(uint8_t channel);
  void stopHPMAParticleMeasurement(uint8_t channel);
  void enableHPMAAutoSend(uint8_t channel);
  void disableHPMAAutoSend(uint8_t channel);
  unsigned int GetHPMAPM2_5(uint8_t channel);
  unsigned int GetHPMAPM10(uint8_t channel);
